@props(['services', 'therapists', 'availablePromotions', 'modalId' => 'book-appointment', 'defaultServiceId' => ''])

<x-modal name="{{ $modalId }}" :show="$errors->any() && old('_modal_id') === $modalId" maxWidth="3xl">
    <div x-data="bookingWizard('{{ old('service_id', $defaultServiceId) }}')" class="p-6 md:p-8">
        <div class="flex justify-between items-center mb-6">
            <h2 class="font-semibold text-xl text-spa-charcoal">
                {{ __('Book an Appointment') }}
            </h2>
            <button x-on:click="$dispatch('close-modal', '{{ $modalId }}')" class="text-spa-gray hover:text-spa-charcoal">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
            </button>
        </div>

        <form action="{{ route('customer.bookings.store') }}" method="POST" @submit="submitForm">
            @csrf
            ...
            @if ($errors->any() && old('_modal_id') === $modalId)
                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div>
                            ...
                            <x-text-input id="appointment_date" name="appointment_date" type="date" class="mt-1 block w-full border-spa-wood focus:border-[#2c3e38] focus:ring-[#2c3e38]" x-model="appointmentDate" required min="{{ date('Y-m-d') }}" />
                        </div>
                        <div>
                            <x-input-label for="start_time" value="Start Time *" />
                            <x-text-input id="start_time" name="start_time" type="time" class="mt-1 block w-full border-spa-wood focus:border-[#2c3e38] focus:ring-[#2c3e38]" x-model="startTime" required />
                        </div>
                    </div>
            ...
            <div x-show="step === 3" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 transform scale-95" x-transition:enter-end="opacity-100 transform scale-100" style="display: none;">
                ...
                <div class="space-y-5">
                    <div>
                        ...
                        <x-text-input type="text" class="mt-1 block w-full bg-spa-cream text-spa-gray opacity-80 cursor-not-allowed border-spa-wood" value="{{ auth()->user()->name }}" disabled />
                    </div>

                    <div>
                        <x-input-label value="Email" />
                        <x-text-input type="text" class="mt-1 block w-full bg-spa-cream text-spa-gray opacity-80 cursor-not-allowed border-spa-wood" value="{{ auth()->user()->email }}" disabled />
                    </div>

                    <div>
                        <x-input-label for="customer_phone" value="Phone Number *" />
                        <x-text-input id="customer_phone" name="customer_phone" type="text" class="mt-1 block w-full border-spa-wood focus:border-[#2c3e38] focus:ring-[#2c3e38]" x-model="customerPhone" required placeholder="09xxxxxxxxx" />
                    </div>
            ...
            <div x-show="step === 4" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 transform scale-95" x-transition:enter-end="opacity-100 transform scale-100" style="display: none;">
                ...
                <div class="bg-spa-cream p-6 rounded-lg border border-spa-beige space-y-4">
                    ...
                    <div class="flex justify-between border-b border-spa-beige pb-3" x-show="selectedPromoId" style="display: none;">
                    ...
                    <div class="pt-2 pb-4">
                        ...
                        @if(count($availablePromotions) > 0)
                            ...
                            <p class="text-xs text-spa-gray opacity-80 mt-2" x-show="promoError" x-text="promoError" style="color: red; display: none;"></p>
                        @else
                            <div class="mt-1 text-sm text-spa-gray opacity-80 bg-spa-beige p-3 rounded-md border border-spa-wood">
                    ...
                    <div class="pt-4">
                        <div class="flex items-center text-sm text-yellow-800 bg-yellow-50 p-3 rounded-md border border-yellow-200">
                            ...
                            <p>Payment is recorded as over-the-counter cash for capstone testing.</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="mt-8 pt-6 border-t border-spa-beige flex justify-between">
                <button type="button" x-show="step > 1" @click="step--" class="px-4 py-2 bg-spa-white border border-spa-wood rounded-md font-semibold text-xs text-spa-charcoal opacity-90 uppercase tracking-widest shadow-sm hover:bg-spa-beige transition ease-in-out duration-150" style="display: none;">
                    Back
                </button>
...
<!-- Only register the script once globally if included multiple times -->
@pushonce('scripts')
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('bookingWizard', (defaultServiceId = '') => ({
            step: 1,
            stepLabels: ['Service', 'Date & Time', 'Details', 'Payment'],
            selectedService: '{{ old('service_id') }}' || defaultServiceId,
            serviceName: '',
            serviceDuration: 0,
            servicePrice: 0,
            ...
            init() {
                if (this.selectedService) {
                    // Try to find the radio button and trigger updateServiceInfo manually or pre-fill it
                    this.$nextTick(() => {
                        const rb = document.querySelector(`input[name="service_id"][value="${this.selectedService}"]`);
                        if (rb) {
                            rb.checked = true;
