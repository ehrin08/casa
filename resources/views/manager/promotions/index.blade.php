<x-manager-layout>
    ...
    <div class="mb-6 flex justify-between items-center">
        <p class="text-gray-500 text-sm">Manage RFM-driven discounts, targeted offers, and off-peak promotion rules.</p>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <a href="{{ route('manager.promotions.rules') }}" class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex items-center hover:shadow-md transition-shadow group">
            ...
            <div>
                ...
                <h3 class="text-2xl font-bold text-gray-800">{{ $activeRulesCount }}</h3>
            </div>
        </a>

        <a href="{{ route('manager.promotions.customer-promotions') }}" class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex items-center hover:shadow-md transition-shadow group">
            <div class="p-3 rounded-full bg-indigo-50 text-indigo-600 mr-4 group-hover:scale-110 transition-transform">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
            </div>
            <div>
                <p class="text-sm font-medium text-gray-500">Generated Codes</p>
                <h3 class="text-2xl font-bold text-gray-800">{{ $generatedCount }}</h3>
            </div>
        </a>

        <a href="{{ route('manager.promotions.customer-promotions', ['status' => 'used']) }}" class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex items-center hover:shadow-md transition-shadow group">
            ...
            <div>
                <p class="text-sm font-medium text-gray-500">Used Promotions</p>
                <h3 class="text-2xl font-bold text-gray-800">{{ $usedCount }}</h3>
            </div>
        </a>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex items-center">
            ...
            <div>
                <p class="text-sm font-medium text-gray-500">RFM Profiles</p>
                <h3 class="text-2xl font-bold text-gray-800">{{ $rfmAnalyzedCount }}</h3>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center">
                <h3 class="text-lg font-bold text-gray-800">Recent Customer Promotions</h3>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100">
                <h3 class="text-lg font-bold text-gray-800">RFM Segment Distribution</h3>
            </div>
            <div class="p-6">
                        ...
                        @foreach($segmentDistribution as $dist)
                            <li class="flex items-center justify-between">
                                <div class="flex items-center gap-3">
                                    ...
                                    <span class="text-sm font-medium text-gray-700 capitalize">{{ str_replace('_', ' ', $dist->segment) }}</span>
                                </div>
                                <span class="text-sm font-bold text-gray-900">{{ $dist->total }}</span>
                            </li>
                        @endforeach
                    </ul>
                @endif
