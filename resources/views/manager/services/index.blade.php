        ...
        <div class="p-6 border-b border-spa-beige bg-gray-50/50">
            <form method="GET" action="{{ route('manager.services.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div>
                    ...
                    <select name="category" id="category" class="block w-full py-2 pl-3 pr-10 border border-spa-wood bg-spa-white rounded-md shadow-sm focus:outline-none focus:ring-[#2c3e38] focus:border-[#2c3e38] sm:text-sm">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-spa-cream">
                    <tr>
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-spa-gray opacity-80 uppercase tracking-wider">Service Name</th>
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-spa-gray opacity-80 uppercase tracking-wider">Category</th>
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-spa-gray opacity-80 uppercase tracking-wider">Price / Duration</th>
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-spa-gray opacity-80 uppercase tracking-wider">Commission Rate</th>
                    ...
                    @forelse ($services as $service)
                        <tr class="hover:bg-spa-beige transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-spa-charcoal">{{ $service->name }}</div>
                                @if($service->description)
                                    <div class="text-sm text-spa-gray opacity-80 truncate max-w-xs">{{ Str::limit($service->description, 50) }}</div>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-spa-charcoal font-semibold">${{ number_format($service->price, 2) }}</div>
                                <div class="text-xs text-spa-gray opacity-80">{{ $service->duration_minutes }} mins</div>
                            </td>
