...
<x-manager-layout>
    ...
    <div class="mb-8">
        <div class="flex justify-between items-center mb-4">
            <h2 class="text-xl font-medium text-spa-charcoal">Analytics Summary</h2>
        ...
        <div class="lg:col-span-2 space-y-8">
            ...
            <x-dashboard.section-card title="Today's Appointments" action="View All Bookings" actionUrl="{{ route('manager.bookings.index') }}">
                @if($pendingBookings > 0)
                    <div class="flex items-center justify-between py-4 border-b border-spa-beige">
                        <div class="flex items-center">
                            <div class="h-10 w-10 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center font-bold">
                                {{ $pendingBookings }}
                            </div>
                            <div class="ml-4">
                                <p class="text-sm font-medium text-spa-charcoal">Appointments scheduled for today</p>
                                <p class="text-xs text-spa-gray opacity-80">Go to Appointments module to manage them.</p>
                            </div>
                        </div>
                        <a href="{{ route('manager.bookings.index') }}" class="px-3 py-1 bg-spa-white border border-spa-wood rounded-md text-xs font-medium text-spa-charcoal opacity-90 hover:bg-spa-beige">Manage</a>
                    </div>
                @else
                    <div class="flex flex-col items-center justify-center py-10 text-spa-gray opacity-80 bg-spa-cream rounded-xl border border-dashed border-spa-beige">
                    ...
                    @forelse($recentTransactions as $tx)
                        <div class="flex items-start">
                            ...
                            <div class="ml-4 flex-1">
                                <p class="text-sm font-medium text-spa-charcoal">₱{{ number_format($tx->total_amount, 2) }} - {{ $tx->customer ? $tx->customer->name : 'Walk-in' }}</p>
                                <p class="text-xs text-spa-gray opacity-80">{{ $tx->created_at->diffForHumans() }} ({{ $tx->payment_status }})</p>
                            </div>
                        </div>
                    @empty
                        <p class="text-sm text-spa-gray opacity-80">No recent transactions.</p>
                    @endforelse
                </div>
            </x-dashboard.section-card>
        </div>
